...
class ExponentialDecay(tf.keras.callbacks.Callback):
    def __init__(self, decay_rate, decay_steps, minimal_lr=1e-6, staircase=False, verbose_steps=None):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        if self.verbose_steps is not None and epoch == 0:
            print('Exponential decay: base learning rate {} minimal learning rate {}.'.format(self.base_lr,
                                                                                              self.minimal_lr))
    ...
    def on_batch_begin(self, batch, logs=None):
        ...
        lr = max(self.base_lr * (self.decay_rate ** power), self.minimal_lr)
